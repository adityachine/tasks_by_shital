...
selected_columns_df = merged_df[[
    "Override Address 3", "Country (Group)", "Region Code (Group)", "Product Description", "Line Of Business",
    "Dell Internal Notes", "Denial Line of Business", "Denial Reason2", "Denial Category", "Auto Dispatch",
    "Swivel Seat Reason", "Swivel Seat Reason Category", "Submitter", "Assigned", "Battery Damaged", "BIL Response",
    "Denial Reason", "OSP", "Date"
]]
...
master_output = 'Master file.xlsx'
selected_columns_df.to_excel(master_output)

"""with pd.ExcelWriter(master_output, mode='a', engine='openpyxl', if_sheet_exists='replace') as writer:
    selected_columns_df.to_excel(writer, sheet_name='Merged_Data', index=False)
"""
